...
function Btn_Features()
{
    var work_pic_frame,Works_Element_Control = 1;

    document.getElementById("APP_works").innerHTML = "<img class='worksimg' src='static/images/apppic.jpg' style='cursor: pointer;'></img>"
    document.getElementById("WEB_works").innerHTML = "<img class='worksimg' src='static/images/webpic.jpg' style='cursor: pointer;'></img>"
    document.getElementById("SOFTWARE_works").innerHTML = "<img class='worksimg' src='static/images/softwarepic.jpg' style='cursor: pointer;'></img>"
    document.getElementById("Back_btn").innerHTML = "<div id='Backbtn' style='cursor:pointer;position:fixed'>返回</div>"

    function build_element(type)
    {
        ...
        document.getElementById("work_pic_frame").innerHTML = KWCworks(type)
        for(let i = 0;i<Image_information_list.length;i++)
        {
            ...
            $("div#frame"+i).click(()=>
            {
                ...
                // scaletoFullScreen(i)
                // window.location.assign("http://kwcstudio.bounceme.net/test/workdetail?ID="+Image_information_list[i].getId)
                window.open("https://kwcstudio.bounceme.net/workdetail?ID="+Image_information_list[i].getId)
            })
        }
        $('html,body').animate({scrollTop:$('#work_pic_frame').offset().top}, 500);
    ...
    $("#APP_works").click(()=>
    {
        ...
        //  APP
        $("#WEB_works,#SOFTWARE_works").hide(300,()=>{(Works_Element_Control == 1)?build_element("APP"):Works_Element_Control = 2})
    })
    ...
    $("#WEB_works").click(()=>
    {
        if(Works_Element_Control==1)CURRENT_PROCESS_PAGE = 2
        //  WEB
        $("#APP_works,#SOFTWARE_works").hide(300,()=>{(Works_Element_Control == 1)?build_element("WEB"):Works_Element_Control = 2})
    })
    $("#SOFTWARE_works").click(() =>
    {
        if(Works_Element_Control==1)CURRENT_PROCESS_PAGE = 2
        //  software
        $("#WEB_works,#APP_works").hide(300,()=>{(Works_Element_Control == 1)?build_element("SOFTWARE"):Works_Element_Control = 2})
...
//  show KWC works  
function KWCworks(type=None,string = "",domain = document.location.href)
{
     // APP add method
    if(type == "APP")string+=sendtoKWCworks_APP()
    //  WEB add method
    else if(type == "WEB")string+=sendtoKWCworks_WEB()
    //  Software add method
    else if(type == "SOFTWARE")string += sendtoKWCworks_SOFTWARE()
